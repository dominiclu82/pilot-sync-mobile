    ...
    <nav class="tab-nav">
      ...
      <div class="tab-controls">
        ...
        <div class="theme-seg" title="日 / 夜">
          ...
          <button class="theme-seg-opt" type="button" onclick="setPortfolioTheme('light')" aria-label="日間 Day">☀️</button>
          <button class="theme-seg-opt" type="button" onclick="setPortfolioTheme('dark')" aria-label="夜間 Night">🌙</button>
        </div>
...
.hdr-btn-font {
  ...
  overflow: hidden; background: var(--bg-card); gap: 0;
}
.hdr-btn-font button {
  flex: 1; display: flex; align-items: center; justify-content: center;
  background: none; border: 0; color: var(--fg); padding: 0;
...
.theme-seg{position:relative;display:inline-flex;flex-direction:column;box-sizing:border-box;width:34px;height:42px;padding:2px;border-radius:5px;background:var(--bg-card);border:1px solid var(--border);cursor:pointer;flex:0 0 auto;margin-right:4px}
...
function setPortfolioTheme(mode) {
  try { localStorage.setItem(THEME_KEY, mode === 'light' ? 'light' : 'dark'); } catch {}
  applyTheme();
}
function toggleTheme() {
  setPortfolioTheme(readTheme() === 'light' ? 'dark' : 'light');
}
