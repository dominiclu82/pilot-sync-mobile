...
function _atisPickKind(msgs: any[], kind: 'ARR' | 'DEP'): { title: string; text: string; src: string; time: string; issueAt: number; receivedAt: number; hash: string } | null {
  const tag = kind + ' ATIS';
  // 必須有「ATIS 字母碼」（tag 後緊跟一個大寫字母）才算有效現行 ATIS。
  // 排除「ARR ATIS NOT AVAILABLE / ARR ATIS \n 2000Z」這種「沒字母碼」的佔位訊息（它 issueScore 解不出發布時刻→退用收到時間→被誤挑為最新；實證 RJAA 踩過）。
  // ⚠ 只看「tag 後有沒有字母碼」，不是濾「內文有沒有 NOT AVBL」——真 ATIS 常含「TWY B NOT AVBL / ILS NOT AVAILABLE」這類設施通告，不可整篇丟掉（codex 指正）。
  const tagLetter = new RegExp(tag + '\\s+[A-Z]\\b');
  const matches = msgs.filter((m) => tagLetter.test(String((m && m.text) || '')));
...
// 庫裡的 entry 是否為「有效現行 ATIS」(tag 後有字母碼)。濾掉 V9.4.23 之前就存進 DB 的「ARR ATIS NOT AVAILABLE」佔位垃圾——
//   merge 只升級不會自動蓋掉、且該站當下無更新的 ATIS 可取代它 → 必須在「讀/載」時擋掉，否則一直顯示舊垃圾（實證 RJAA 踩過）。
function _atisEntryHasLetter(kind: 'ARR' | 'DEP', text: string): boolean {
  return new RegExp(kind + ' ATIS\\s+[A-Z]\\b').test(String(text || ''));
}
...
function _atisStoreSections(icao: string): { title: string; text: string; src: string; time: string }[] | null {
  ...
  for (const kind of ['ARR', 'DEP'] as const) {
    ...
    if (!_atisEntryHasLetter(kind, v.text)) continue;   // 濾掉佔位垃圾（NOT AVAILABLE 等無字母碼的舊 entry）
...
async function _atisStoreLoad() {   // 開機從 DB 載回累積庫 → Render 重啟/部署不歸零（跟 coffee 一樣留得住）
  ...
  try {
    ...
    for (const r of rr.rows) {
      ...
      if (!_atisEntryHasLetter(r.kind, r.text)) continue;   // 不載入佔位垃圾（舊版存的 NOT AVAILABLE 等）
