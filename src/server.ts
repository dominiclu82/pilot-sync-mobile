...
function _atisPickKind(msgs: any[], kind: 'ARR' | 'DEP' | 'ATIS'): { title: string; text: string; src: string; time: string; issueAt: number; receivedAt: number; hash: string } | null {
  ...
  // 真實發布時刻 realIssueAt：
  //   ① 優先抓內含 METAR 觀測時戳「DDHHMMZ」(6 碼，含日期) —— 日本場可靠（字母後那串 HHMM 不是發布時刻），跨月也算得對。
  //   ② 沒 METAR → 用「字母後 HHMMZ + 收到日期」推算（台灣場：字母後那串就是發布時刻；發布晚於收到＝前一天發的，推回一天）。
  //   ③ 都解不出 → 退用收到時間。
  // ⚠ 不再因為「太舊」就丟掉（原本 >8hr 丟成 -Infinity 害日本 DEP 整個變空）。改成：照算真實時刻，舊也照顯示、用真實時刻誠實標時間。
  const realIssueAt = (m: any): number => {
    const t = String((m && m.text) || '');
    const ing = Date.parse(String((m && m.timestamp) || '')) || Date.now();
    const d = new Date(ing);
    const lm = t.match(/ATIS\s+[A-Z]\s+(\d{2})(\d{2})Z/);   // 字母後 HHMM = 真正「發布時分」（精準，能區分「同 METAR 不同字母」的兩則，codex P1）
    const mm = t.match(/\b(\d{2})(\d{2})(\d{2})Z\b/);         // METAR DDHHMM = 可靠的「日」（日本場字母後時間不是發布時刻，但日要靠 METAR）
    const dd = mm ? parseInt(mm[1], 10) : 0;
    if (lm) {
      const hh = parseInt(lm[1], 10), mi = parseInt(lm[2], 10);
      if (mm && dd >= 1 && dd <= 31) {
        // 有 METAR：用「METAR 的日」+「ATIS 的時分」。
        const mh = parseInt(mm[2], 10), mmin = parseInt(mm[3], 10);
        let ts = Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), dd, hh, mi);
        if (ts > ing + 6 * 3600000) ts = Date.UTC(d.getUTCFullYear(), d.getUTCMonth() - 1, dd, hh, mi);   // DD 屬上個月（月初收到、月底觀測）
        if ((mh * 60 + mmin) - (hh * 60 + mi) > 12 * 60) ts += 86400000;   // METAR 在深夜、ATIS 在凌晨 → ATIS 發布跨到隔天
        return ts;
      }
      // 無 METAR（台灣場：字母後時間就是發布時刻）→ 收到日 + ATIS 時分，發布晚於收到＝前一天發的。
    ...
    return ing;   // 都解不出 → 收到時間
  };
  // 挑「真實發布時刻最新」的那則。⚠ 永不變空：只要有 matches 就一定回一則（舊也照顯示，使用者明確要求「有就顯示、別濾成空」）。
  ...
  for (const m of matches) { const s = realIssueAt(m); if (s <= future && s > bestScore) { best = m; bestScore = s; } }
  ...
  // time 顯示「真實發布時刻」（METAR/推算），不是收到時間 → 舊資料一眼看出是舊的，不再「收到 2249 卻顯示 2138」那種矛盾。
  return { title: tag, text, src: _atisSrcOf(best), time: _atisFmtTime(new Date(bestScore).toISOString()), issueAt: bestScore, receivedAt, hash: _atisHash(text) };
}
...
function _atisStoreSections(icao: string): { title: string; text: string; src: string; time: string }[] | null {
  ...
  for (const kind of ['ARR', 'DEP', 'ATIS'] as const) {
    ...
    out.push({ title: kind === 'ATIS' ? 'ATIS' : kind + ' ATIS', text: v.text, src: v.src, time: _atisFmtTime(new Date(v.issueAt).toISOString()) });
  }
...
async function _atisStoreLoad() {   // 開機從 DB 載回累積庫 → Render 重啟/部署不歸零（跟 coffee 一樣留得住）
  ...
  try {
    ...
    for (const r of rr.rows) {
      ...
      cur[r.kind as 'ARR' | 'DEP' | 'ATIS'] = { text: r.text, src: r.src || '', issueAt: Number(r.issue_ms) || 0, receivedAt: Number(r.recv_ms) || 0, hash: r.text_hash || '', updatedAt: Date.now() };
