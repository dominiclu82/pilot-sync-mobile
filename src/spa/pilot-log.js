...
function _plGenLogbookPdf(formatId, opts) {
  ...
  _plLoadJsPdf().then(function () {
    ...
    fontP.then(function (fontB64) {
      ...
      // ── 封面（中性持有人前頁，不抄 LogTen）；可選開關：封面整頁 / License 行 / Total Time 行 ──
      if (opts.cover !== false) {
        var cy = pageH / 2 - 28;
        doc.setFontSize(20); _plLbFont(doc, 'PILOT LOGBOOK', CJK, 'bold'); doc.setTextColor(20);
        doc.text('PILOT LOGBOOK', pageW / 2, cy, { align: 'center' });
        doc.setFontSize(12); _plLbFont(doc, '飛行紀錄簿', CJK, 'normal'); doc.setTextColor(90);
        doc.text('飛行紀錄簿', pageW / 2, cy + 8, { align: 'center' });
        doc.setDrawColor(170); doc.setLineWidth(0.3); doc.line(pageW / 2 - 55, cy + 14, pageW / 2 + 55, cy + 14);
        doc.setFontSize(18); _plLbFont(doc, pilot, CJK, 'bold'); doc.setTextColor(20);
        doc.text(pilot || '', pageW / 2, cy + 28, { align: 'center' });
        doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(70);
        var ly = cy + 40;
        if (opts.license !== false) { doc.text('License No.  __________________________', pageW / 2, ly, { align: 'center' }); ly += 8; }
        doc.text('Period:  ' + r.from + '   —   ' + r.to, pageW / 2, ly, { align: 'center' }); ly += 8;
        if (opts.totalTime !== false) {
          ...
          doc.text('Total Time:   this period ' + _plMinToHHMM(periodMin) + '    ·    to date ' + _plMinToHHMM(cumMin), pageW / 2, ly, { align: 'center' });
        }
        doc.setTextColor(0);
      }
      ...
      function totalsRows(cols, offset, pageSum, brought, toDate, withLabel) {
        ...
        function mk(label, vals, isCum, ri) {
          ...
          for (var ci = startCi; ci < cols.length; ci++) {
            ...
            if (ci === rc) {
              if (ri === 0) row.push({ content: CERT.join('\n'), rowSpan: 3, styles: { halign: 'left', valign: 'top', fontStyle: 'normal', fillColor: [255, 255, 255] } });
      ...
      function renderPhys(cols, groups, offset, chunk, pageSum, brought, toDate, withLabel, pageLabel) {
        ...
        doc.autoTable({
          head: head, body: body, startY: 12, theme: 'grid',
          ...
          headStyles: { fillColor: [40, 60, 90], textColor: 255, fontSize: hfs, minCellHeight: rowH, halign: 'center', fontStyle: 'bold', overflow: 'ellipsize', valign: 'middle' },
