...
// V2.4.xx：日夜改直立分段膠囊（☀️上/🌙下 + 浮標）。浮標位置純 CSS 靠 [data-theme] 驅動，
// 這裡只設模式。_plSetTheme 設成指定模式；_plToggleTheme 保留相容（給舊呼叫者）。
function _plSetTheme(mode) {
  var html = document.documentElement;
  if (mode === 'light') { html.dataset.theme = 'light'; try { localStorage.setItem('pilotlog_theme', 'light'); } catch (e) {} }
  else { delete html.dataset.theme; try { localStorage.setItem('pilotlog_theme', 'dark'); } catch (e) {} }
}
function _plToggleTheme() { _plSetTheme(document.documentElement.dataset.theme === 'light' ? 'dark' : 'light'); }
...
function switchPlTab(tab, btn) {
  ...
  // V2.4.14：自動存模型 —— 切走前先把未存編輯 flush 落地（此時編輯器 DOM 還在，讀得到表單），不丟資料。
...
function _plReconcileId(localId, realId) {
  ...
  if (_pl.aircraftEntries) for (var a = 0; a < _pl.aircraftEntries.length; a++) if (_pl.aircraftEntries[a].id === localId) _pl.aircraftEntries[a].id = realId;
  if (_pl.selectedId === localId) _pl.selectedId = realId;
  // V2.4.14：自動存的新航班開著編輯器時 temp→正式 id，editing 也要換，否則 Lock/Delete 仍對舊 local id 動作（codex P2）
  if (_pl.editing && _pl.editing.id === localId) {
    _pl.editing.id = realId;
    var hd = document.getElementById('ple-editor-head');
    if (hd) hd.innerHTML = _plEditorHeadInner(_pl.editing, !!(document.getElementById('pl-detail-pane') && document.getElementById('pl-detail-pane').contains(hd)));
  }
...
function _plRenderToolbar() {
  ...
  return '<div style="margin-bottom:10px">' +
    '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px">' +
      ...
      // V2.4.xx：ICAO/IATA 橫式分段切換（亮的＝目前格式、浮標左右滑），取代原單鈕顯示目標
      '<div class="code-seg' + (_plAptFmtCur() === 'iata' ? ' is-iata' : '') + '" title="機場碼顯示格式（亮起＝目前）">' +
        '<span class="code-seg-knob"></span>' +
        '<span class="code-seg-opt' + (_plAptFmtCur() === 'iata' ? '' : ' on') + '" onclick="_plSetAptFmt(\'icao\')">🌐 ICAO</span>' +
        '<span class="code-seg-opt' + (_plAptFmtCur() === 'iata' ? ' on' : '') + '" onclick="_plSetAptFmt(\'iata\')">IATA</span>' +
      '</div>' +
      // V1.3.33：一鍵上鎖 / 解鎖全部航班
...
function _plRenderEntryRow(e) {
  ...
  // V2.4.xx：地面/模擬機本來就沒有 OOOI，不該掛「缺資料」扳手 🛠（它們已有 GND/SIM 標籤）。
  var acIcon = e.is_deadhead ? '🧳' : e.is_sim ? '🖥️' : e.is_ground ? '🏢' : ((e.out_utc && e.in_utc) ? '✈' : '🛠');
...
function _plOpenEditor(id) {
  // V2.4.14：直接切到另一筆（不經 _plCloseEditor）時，先把前一筆的未存改動 flush 落地，再重置自動存狀態（codex P1）。
  if (_pl._autoSaveTimer) { clearTimeout(_pl._autoSaveTimer); _pl._autoSaveTimer = null; }
  if (_pl._editorDirty && _pl.editing && !_pl.editing.is_locked) { try { _plSaveEntry({ keepOpen: true }); } catch (e) {} }
  _pl._editorDirty = false;
...
function _plCloseEditor() {
  // V2.4.14：關閉前 flush —— 有任何未存的改動（dirty，含程式改值與防抖未到的輸入）就先落地，確保不丟資料。
  if (_pl._autoSaveTimer) { clearTimeout(_pl._autoSaveTimer); _pl._autoSaveTimer = null; }
  if (_pl._editorDirty && _pl.editing && !_pl.editing.is_locked) { try { _plSaveEntry({ keepOpen: true }); } catch (e) {} }
  _pl._editorDirty = false;
...
function _plEditorField(label, name, type, opts) {
  ...
  if (type === 'time-utc') {
    ...
    // V2.4.xx：remarks 隨輸入自動長高（oninput → _plAutoGrow）；rows=2 當最小高
    input = '<textarea ' + attrs + ' rows="2" oninput="_plAutoGrow(this)">' + _plEsc(val) + '</textarea>';
  } else if (type === 'number') {
...
function _plCrewField(key, e, labelOverride) {
  ...
  // V2.4.xx：已上鎖航班不可拖拉換位（鎖定＝不可改動）。
  var drag = (key === 'pic' || key === 'crew2' || key === 'crew3' || key === 'crew4') && !(_pl.editing && _pl.editing.is_locked);
...
function _plCrewPick(key, cid) {
  ...
  if (typeof _plScheduleAutoSave === 'function') _plScheduleAutoSave();   // V2.4.14：下拉選組員是程式改值 → 觸發自動存
}
...
function _plSwapPicSic() {
  ...
  if (typeof _plScheduleAutoSave === 'function') _plScheduleAutoSave();   // V2.4.14：程式改值 → 觸發自動存（input 事件不會自動發）
...
function _plCkDragStart(ev, key) {
  if (_pl.editing && _pl.editing.is_locked) return;   // V2.4.xx：上鎖不可拖拉換位（保險，把手本來就不顯示）
...
function _plCkReorder(srcKey, tgtKey) {
  ...
  _plUpdateOwnerRole();
  if (typeof _plScheduleAutoSave === 'function') _plScheduleAutoSave();   // V2.4.14：拖拉換位 → 觸發自動存
}
...
// V2.4.xx：改成橫式分段切換（亮的＝目前格式）→ 直接設定指定格式，不再單純 toggle。
function _plSetAptFmt(fmt) {
  fmt = (fmt === 'iata') ? 'iata' : 'icao';
  if (_plAptFmtCur() === fmt) return;   // 已是該格式 → 不動
  try { localStorage.setItem('pilotlog_apt_fmt', fmt); } catch (e) {}
  _plToast('機場碼顯示：' + fmt.toUpperCase());
...
function _plToggleAptFmt() { _plSetAptFmt(_plAptFmtCur() === 'iata' ? 'icao' : 'iata'); }   // 相容舊呼叫
// 編輯器開著時切換 IATA/ICAO → 只同步 From/To（其他欄位不動、不丟編輯）：轉碼 + 改 label + 重建星宇下拉。
// V2.4.xx：textarea 隨內容自動長高（remarks 用）。設 auto 再吃 scrollHeight，避免只增不減。
function _plAutoGrow(el) {
  if (!el) return;
...
function _plSyncRouteFmt() {
  ...
  [['origin', 'From'], ['dest', 'To']].forEach(function(p) {
    ...
    // V2.3（codex P3）：若使用者自訂了 From/To 名稱，尊重它；沒自訂才用預設。
    // V2.4.xx：拿掉「(ICAO/IATA)」字眼 —— 欄位本就同時吃 ICAO/IATA，標格式反而誤導。
    if (lb) { var cl = _plFieldLabel(p[0], ''); lb.textContent = cl ? cl : p[1]; }
  });
  _plUpdateAptLists();
}
...
function _plWireEditor() {
  ...
  // V2.4.xx：開啟時依現有 remarks 內容把欄位撐到合適高度（鎖定的也要撐開才看得到全文）
  try { var _rmkTa = document.getElementById('ple-remarks'); if (_rmkTa) _plAutoGrow(_rmkTa); } catch (e) {}
...
// V2.4.14：編輯器標題列內容（關閉鈕 + 標題 + 狀態 + Fields/Return/Lock/自動存提示/Delete）。
...
function _plRenderEditor(target) {
  ...
  c.innerHTML =
    ...
    '<div style="background:var(--card);border-radius:10px;padding:12px">' +
      ...
      '<div id="ple-route-row" style="display:' + ((_plEntryType(e) === 'sim' || _plEntryType(e) === 'ground') ? 'none' : '') + '">' + _plFieldRow(2, _plEditorField('From', 'origin', 'text', { fmt: _plAptFmt, aptSide: 'origin', labelId: 'ple-origin-label', placeholder: '選星宇航點或輸入 · pick/type' }) + _plEditorField('To', 'dest', 'text', { fmt: _plAptFmt, aptSide: 'dest', labelId: 'ple-dest-label', placeholder: '選星宇航點或輸入 · pick/type' })) +
  ...
  // V2.4.14：LogTen 式自動存 —— 編輯器內任何輸入/變更 → 防抖 800ms 後背景存（不關閉、不丟焦點），不用 Save 鈕。
  //   上鎖的不掛（欄位 disabled）。程式改值的 helper（Swap PIC/SIC、拖拉換位…）改呼叫 _plScheduleAutoSave（input 事件不會自動發）。
  if (c && !e.is_locked) {
    c.addEventListener('input', _plScheduleAutoSave);
    c.addEventListener('change', _plScheduleAutoSave);
  }
}
// V2.4.14：統一的自動存排程入口 —— 標 dirty + 重置 800ms 防抖。input/change 事件與程式改值 helper 都呼叫這個。
function _plScheduleAutoSave() {
  ...
  _pl._autoSaveTimer = setTimeout(function () {
    ...
    if (_pl.editing && !_pl.editing.is_locked) { try { _plSaveEntry({ keepOpen: true }); } catch (e2) {} }
  }, 800);
}
...
// V2.4.14：LogTen 式自動存 —— opts.keepOpen=true 時「背景存、不關閉編輯器」（給輸入防抖 + 關閉前 flush 用）。
//   沒帶 opts（舊行為）= 存完關閉並重繪（目前已無 UI 呼叫，保留相容）。
async function _plSaveEntry(opts) {
  var keepOpen = !!(opts && opts.keepOpen);
  var e = _pl.editing;
  if (!e) return;
  if (e.is_locked) return;   // 上鎖不存（欄位本來就 disabled，保險）
  // V2.4.14（codex P1）：防抖 timer 可能在「使用者切到別頁、編輯器 DOM 已被換掉」後才發 → 此時讀表單全是空，
  //   會把草稿覆蓋成空白。偵測核心欄位不在 DOM → 中止（不讀、不寫），避免清空資料。
  if (keepOpen && !document.getElementById('ple-flight_date')) return;
  ...
  // V2.4.14：新航班的建立由 _editorDirty 把關 —— 自動存只在「使用者真的改過」時才會被呼叫（_plScheduleAutoSave
  //   設 dirty、close/switch 也只在 dirty 時 flush），所以走到這裡＝有實質編輯，直接建即可（不再用脆弱的欄位空白清單，
  //   那會誤殺只填 position／PF／日期 等的部分新航班，codex P2）。純開了沒動的 New Entry 不會走到這裡。
  ...
  if (keepOpen) {
    // V2.4.14：自動存 —— 不關閉、不重繪編輯器（保留焦點，使用者繼續打字）。
    if (isNew) {
      e.id = id; e.status = statusForCache; e.source = cached.source;   // 新建後拿到 id → 之後存變 update，不重複 create
      // 只重建「標題列」露出 Delete/Lock/Return（不碰欄位 → 不丟焦點，codex P2）
      var hd = document.getElementById('ple-editor-head');
    ...
    if (_plWide() && document.getElementById('pl-detail-pane') && document.getElementById('pl-list')) _plRenderList();
    _plSync();
    return;
  }
...
async function _plToggleLock() {
  var e = _pl.editing;
  if (!e || !e.id) return;
  // V2.4.14：上鎖前先 flush 未存編輯 —— 否則防抖窗內按 Lock，後續 autosave 會因已鎖定 early-return 而丟掉這些改動（codex P2）
  if (_pl._autoSaveTimer) { clearTimeout(_pl._autoSaveTimer); _pl._autoSaveTimer = null; }
...
async function _plUploadWader(dryRun) {
  ...
  var msg = (dryRun ? '🔍 Dry-run（沒寫入 DB）：' : '✅ 匯入完成：') +
    ...
    (j.oooi_backfilled ? '、<span style="color:#4ade80">回填 OOOI <b>' + j.oooi_backfilled + '</b> 筆</span>' : '');
...
async function _plQuickCreateCrew(slotKey) {
  ...
  if (typeof _plScheduleAutoSave === 'function') _plScheduleAutoSave();   // V2.4.14：回填了組員名/員編 → 觸發自動存
  _plCloseCrewModal();
...
// logbook 持有人姓名：本人通訊錄 display_name 優先，退而求其次用登入 email（_pl.user 沒有 name 欄）
function _plSelfName() {
  var crew = _pl.crew || [];
  for (var i = 0; i < crew.length; i++) { if (crew[i].is_self && (crew[i].display_name || '').trim()) return crew[i].display_name.trim(); }
  return (_pl.user && _pl.user.primaryEmail) || '';
}
// logbook 欄寬：依使用者「實際資料」逐欄量寬 —— 沒資料的欄縮到最窄（只放表頭、可換行），有資料的欄分到剩餘寬度。
//   不刪欄（turboprop 等照留，給小飛機背景的人）；同一程式每位使用者依自己資料自動調寬。avail=可用總寬(mm)。
function _plLbColWidths(cols, src, avail) {
  var meas = cols.map(function (c) {
    var maxBody = 0, hasData = false;
    for (var i = 0; i < src.length; i++) { var s = String(c.get(src[i]) || ''); if (s) { hasData = true; if (s.length > maxBody) maxBody = s.length; } }
    return { hasData: hasData, body: maxBody, head: String(c.h || '').length, headStr: String(c.h || ''), isRem: /remark/i.test(c.h || '') };
  });
  // 地板寬：簽名/認證欄(Remarks) 固定 34mm（即使 remarks 空白也留得下簽名）；有資料欄 6mm；
  //   空欄＝至少容得下表頭「最長單字」（避免在字中間斷行如 Countr/y、Engine/er），最小 4mm、上限 16mm。
  var floors = meas.map(function (m) {
    if (m.isRem) return 34;
    if (m.hasData) return 6;
    var lw = m.headStr.split(/\s+/).reduce(function (mx, w) { return Math.max(mx, w.length); }, 0);
    return Math.max(4, Math.min(17, lw * 1.55 + 3.2));   // 容得下整個最長單字（含 padding），短字如 ASES/AMES 不再斷成單獨一個 S
  });
  var floorSum = floors.reduce(function (a, b) { return a + b; }, 0);
  var extra = Math.max(0, avail - floorSum);
  // 剩餘寬度按「內容權重」分配：空欄幾乎不分(0.3)；Remarks 給足權重；有資料欄依表頭/內容長度（上限 16，避免吃光）。
  var wt = meas.map(function (m) { return m.isRem ? 16 : (m.hasData ? Math.max(m.head * 0.5, Math.min(m.body, 16)) : 0.3); });
  var wtSum = wt.reduce(function (a, b) { return a + b; }, 0) || 1;
  return floors.map(function (f, i) { return f + extra * wt[i] / wtSum; });
}
...
function _plT(h, val) { return { h: h, get: function (e) { var m = val(e); return m > 0 ? _plMinToHHMM(m) : ''; }, val: val, tot: 'time' }; }   // 0 → 空白（照紙本/LogTen，不印 0:00）
...
// 機型分類（給 logbook 的 category/class 欄正確填值，不再一律當噴射機）——
//   先查使用者機型表 pilot_aircraft_types 的 engine_type/class（權威）；查不到用型號樣式推。回 true/false/null（未知→留空）。
function _plAcRec(e) { var t = (e.aircraft_type || '').toUpperCase().replace(/[^A-Z0-9]/g, ''); if (!t) return null; var a = _pl.aircraftTypes || []; for (var i = 0; i < a.length; i++) { if ((a[i].type_code || '').toUpperCase().replace(/[^A-Z0-9]/g, '') === t) return a[i]; } return null; }
function _plAcIsJet(e) {
  var rec = _plAcRec(e); if (rec && rec.engine_type) { var en = String(rec.engine_type).toLowerCase(); if (/jet|fan/.test(en)) return true; if (/turbo|prop|piston/.test(en)) return false; }
  var t = (e.aircraft_type || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (/^(ATR|AT[0-9]|DH|Q[0-9]|SF|SB|BE|B19|C[12]|PA|DA|SR|TBM|PC|DR|CES|BEC|E11|E12|EMB1[12])/.test(t)) return false;   // 螺旋槳/小飛機（含 EMB-110/120 渦輪螺旋槳）
  if (/^(A[123]|B[5-9]|MD|DC|E1[3479]|E29|E2[0-9][0-9]|CRJ|CL|RJ|F[071]|G[0-9]|BCS|BD|GLF|LJ)/.test(t)) return true;        // 噴射客機/公務機（E-Jet/ERJ 噴射：E13x/E14x/E17x/E19x/E29x）
  return null;
}
function _plAcMEL(e) {   // 多發陸 Airplane Multi-Engine Land
  var rec = _plAcRec(e); if (rec && rec.class) { var cl = String(rec.class).toUpperCase(); if (/AMEL/.test(cl) || (/MULTI/.test(cl) && /LAND/.test(cl))) return true; if (/ASEL|ASES|AMES/.test(cl)) return false; }
  if (_plAcIsJet(e) === true) return true;
  var t = (e.aircraft_type || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (/^(ATR|AT[0-9]|DH|Q[0-9]|SF|SB|E12|B19|BE9|BE10|BE20|BE30|BE40)/.test(t)) return true;
  if (/^(C1|C20|PA2|PA3|DA[24]|SR2|TBM|PC12|DR|CES)/.test(t)) return false;
  return null;
}
function _plAcSEL(e) {   // 單發陸 Airplane Single-Engine Land
  var rec = _plAcRec(e); if (rec && rec.class) { var cl = String(rec.class).toUpperCase(); if (/ASEL/.test(cl)) return true; if (/AMEL|AMES|ASES/.test(cl)) return false; }
  if (_plAcIsJet(e) === true || _plAcMEL(e) === true) return false;
  var t = (e.aircraft_type || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (/^(C1|C20|PA2|PA3|DA[24]|SR2|TBM|PC12|DR|CES)/.test(t)) return true;
  return null;
}

// 各格式（欄位設定）。只列「有官方明確範本」的地區 + 通用擺第一。
// V2.4.20：四種標準 logbook 完整版面，照真實紙本「左頁 / 右頁」跨頁排（不再簡化）。
//   每個格式有 left[] / right[]（欄定義 {h,get,val?,tot?}）+ leftGroups[] / rightGroups[]（兩層分組表頭 {l,s}）
//   + totalsLabels[本頁,承前,累計] + certLines[認證三行，放進 REMARKS 欄的結轉列] + dateGet（各家日期格式）。
//   有資料就填、沒有就空白（Dual/Instructor/Solo 等對航線機師不適用 → 空著，跟 LogTen 一樣）。
function _plLbFormats() {
  var pad2 = function (n) { return ('0' + n).slice(-2); };
  var dSlash = function (e) { var p = String(e.flight_date || '').slice(0, 10).split('-'); return p.length === 3 ? p[2] + '/' + p[1] + '/' + p[0].slice(2) : ''; };          // dd/mm/yy
  var dDMY = function (e) { var p = String(e.flight_date || '').slice(0, 10).split('-'); if (p.length !== 3) return ''; var mon = _PL_MON[parseInt(p[1], 10) - 1] || ''; return p[2] + '.' + (mon.charAt(0) + mon.slice(1).toLowerCase()) + '.' + p[0].slice(2); };   // 02.Jan.26
  var dYMD = function (e) { var p = String(e.flight_date || '').slice(0, 10).split('-'); return p.length === 3 ? p[0] + '/' + parseInt(p[1], 10) + '/' + parseInt(p[2], 10) : ''; };   // 2026/1/2
  var dYMDp = function (e) { var p = String(e.flight_date || '').slice(0, 10).split('-'); return p.length === 3 ? p[0] + '/' + p[1] + '/' + p[2] : ''; };   // 2026/01/18（零補位）
  ...
  // 含冒號的 UTC 時刻（00:48），給 OUT/IN/DEP/ARR 時間欄
  var hhmmC = function (ts) { var s = _plUtcHHMM(ts); return s ? s.slice(0, 2) + ':' + s.slice(2) : ''; };
  var outT = function (e) { return hhmmC(e.out_utc || e.off_utc); };
  var inT = function (e) { return hhmmC(e.in_utc || e.on_utc); };
  // 衍生欄（依機型分類，不再一律當噴射機 — codex P1）：JET/AMEL/ASEL 只在機型確實是該類別時填 block；多機組需噴射或 operating_crew≥2。
  //   IFR / cross-country 我們沒有可靠資料來源（不存 IFR；XC 有距離定義）→ 一律留空（LogTen 對航線機師那幾欄也是空的）。
  // 多機組 = 多機組「認證機型」，依機型判定（不是看機上幾人 —— GA 雙人帶飛仍是單駕機；但 ATR/Dash8 等支線渦輪螺旋槳客機
  //   本來就需兩名駕駛＝多機組）。涵蓋：噴射客機/公務機 + 已知雙駕渦輪螺旋槳客機；排除單駕渦輪螺旋槳（King Air/PC-12/TBM）。codex P1。
  var isMP = function (e) {
    if (_plAcIsJet(e) === true) return true;
    var t = (e.aircraft_type || '').toUpperCase().replace(/[^A-Z0-9]/g, '');
    return /^(ATR|AT4|AT5|AT7|DH8|DHC8|Q[0-9]|SF3|S34|S20|SB20|F50|FK50|J41|JS41|D328|DO328|E11|E12|EMB1[12])/.test(t);
  };
  var jetV = function (e) { return _plAcIsJet(e) === true ? blk(e) : 0; };
  var amelV = function (e) { return _plAcMEL(e) === true ? blk(e) : 0; };
  var aselV = function (e) { return _plAcSEL(e) === true ? blk(e) : 0; };
  var mpV = function (e) { return isMP(e) ? blk(e) : 0; };
  // EASA SINGLE-PILOT SE/ME：只在「非多機組」的單駕時間才填（多機組時間歸 Multi-Pilot，SE/ME 空）。GA 單發→SE、雙發→ME。
  var seV = function (e) { return (!isMP(e) && _plAcSEL(e) === true) ? blk(e) : 0; };
  var meV = function (e) { return (!isMP(e) && _plAcMEL(e) === true) ? blk(e) : 0; };
  // approaches：型別+跑道+機場（ILS Z 34R RJAA）；數量
  var apprType = function (e) { var a = Array.isArray(e.approaches) ? e.approaches : []; return a.map(function (x) { return [x.type, x.runway, x.airport].filter(Boolean).join(' '); }).filter(Boolean).join(', '); };
  var apprNum = function (e) { var a = Array.isArray(e.approaches) ? e.approaches : []; return a.reduce(function (s, x) { return s + (x.count || 1); }, 0); };
  var rmkAppr = function (e) { var ap = apprType(e), rm = rmk(e); return ap ? (rm ? ap + ' · ' + rm : ap) : rm; };   // remarks + approach（給沒有獨立 approach 欄的格式）

  return [
    // ── EASA-FCL（AMC1 FCL.050；左頁欄 1–8、右頁欄 9–12）──────────────────────────
    {
      id: 'easa', name: 'EASA-FCL', orientation: 'landscape',
      totalsLabels: ['TOTAL THIS PAGE', 'TOTAL FROM PREVIOUS PAGES', 'TOTAL TIME'],
      certLines: ['I certify that the entries in this log are true.', '________________________________', "PILOT'S SIGNATURE"],
      leftGroups: [
        { l: 'DATE', s: 1 }, { l: 'DEPARTURE', s: 2 }, { l: 'ARRIVAL', s: 2 }, { l: 'AIRCRAFT', s: 2 },
        { l: 'SINGLE-PILOT', s: 2 }, { l: 'MULTIPILOT', s: 1 }, { l: 'TOTAL TIME OF FLIGHT', s: 1 }, { l: 'NAME PIC', s: 1 },
        { l: 'TAKEOFFS', s: 2 }, { l: 'LANDINGS', s: 2 },
      ],
      left: [
        _plX('Date', dSlash),
        _plX('Place', org), _plX('Time', outT), _plX('Place', dst), _plX('Time', inT),
        _plX('Make, Model, Variant', typ), _plX('Registration', reg),
        _plT('SE', seV), _plT('ME', meV),
        _plT('Multi-Pilot', mpV), _plT('Total', blk),
        _plX('Name PIC', _plPicNameCell),
        _plI('Day', dTO), _plI('Night', nTO), _plI('Day', dLd), _plI('Night', nLd),
      ],
      rightGroups: [
        { l: 'OPERATIONAL CONDITION TIME', s: 2 }, { l: 'PILOT FUNCTION TIME', s: 5 },
        { l: 'SYNTHETIC TRAINING DEVICES', s: 3 }, { l: 'REMARKS AND ENDORSEMENTS', s: 1 },
      ],
      right: [
        _plT('Night', ngt), _plX('IFR', blank),
        _plT('PIC P1', _plPicMin), _plX('P1 u/s', blank), _plT('Co-Pilot', _plSicMin), _plX('Dual', blank), _plX('Instructor', blank),
        _plX('Date', blank), _plX('Type', blank), _plX('Total', blank),
        _plX('Remarks', rmkAppr),
      ],
    },

    // ── General（LogTen 的 International 版面；Coradine 是 LogTen 公司名，非格式名）──────
    {
      id: 'general', name: 'General', orientation: 'landscape',
      totalsLabels: ['TOTAL THIS PAGE', 'FORWARDED TOTALS', 'TOTAL TIME'],
      certLines: ['I certify that the entries in this log are true.', '________________________________', "PILOT'S SIGNATURE"],
      leftGroups: [
        { l: 'DATE', s: 1 }, { l: 'AIRCRAFT', s: 2 }, { l: 'PILOT IN COMMAND', s: 1 }, { l: 'ROUTE OF FLIGHT', s: 4 },
        { l: 'FLIGHT NUMBER', s: 1 }, { l: 'TOTAL TIME', s: 1 }, { l: 'MULTIPILOT', s: 1 },
        { l: 'CATEGORY / CLASS', s: 7 }, { l: 'TAKEOFFS', s: 2 }, { l: 'LANDINGS', s: 2 },
      ],
      left: [
        _plX('Date', dYMDp),
        _plX('Type', typ), _plX('Registration', reg),
        _plX('Pilot in Command', _plPicNameCell),
        _plX('From', org), _plX('Out', outT), _plX('In', inT), _plX('To', dst),
        _plX('Flight No.', fno),
        _plT('Total', blk), _plT('Multi-Pilot', mpV),
        _plT('ASEL', aselV), _plX('ASES', blank), _plT('AMEL', amelV), _plX('AMES', blank), _plT('JET', jetV), _plX('Turboprop', blank), _plX('Rotorcraft', blank),
        _plI('Day', dTO), _plI('Night', nTO), _plI('Day', dLd), _plI('Night', nLd),
      ],
      rightGroups: [
        { l: 'CONDITIONS OF FLIGHT', s: 5 }, { l: 'APPROACHES', s: 2 }, { l: 'HOLDS', s: 1 }, { l: 'SIMULATOR', s: 1 },
        { l: 'TYPE OF PILOTING TIME', s: 6 }, { l: 'REMARKS', s: 1 },
      ],
      right: [
        _plX('Cross Country', blank), _plT('Night', ngt), _plX('Solo', blank), _plX('Actual IFR', blank), _plX('Simulated IFR', blank),
        _plX('Type', apprType), _plI('Number', apprNum),
        _plX('Holds', blank), _plX('Simulator', blank),
        _plX('As Flight Instructor', blank), _plX('Flight Engineer', blank), _plT('PIC', _plPicMin), _plT('Co-Pilot', _plSicMin), _plX('Relief', blank), _plX('Dual Received', blank),
        _plX('Remarks', rmk),
      ],
    },
    ...
    {
      id: 'asa', name: 'ASA Standard', orientation: 'landscape',
      totalsLabels: ['Page Total', 'Amount Forward', 'Totals to Date'],
      certLines: ['I certify that the statements made by me on this form are true.', '________________________________', "Pilot's Signature:"],
      leftGroups: [
        { l: 'Date', s: 1 }, { l: 'Aircraft Make and Model', s: 1 }, { l: 'Aircraft Ident', s: 1 }, { l: 'Points of Departure & Arrival', s: 2 },
        { l: 'Flight No.', s: 1 }, { l: 'Remarks, Procedures, Maneuvers, Endorsements', s: 1 }, { l: 'Landings', s: 2 }, { l: 'Aircraft Category', s: 4 },
      ],
      left: [
        _plX('Date', dYMD),
        _plX('Make and Model', typ), _plX('Ident', reg),
        _plX('From', org), _plX('To', dst),
        _plX('Flight No.', fno),
        _plX('Remarks', rmkAppr),
        _plI('Day', dLd), _plI('Night', nLd),
        _plT('Airplane', blk), _plX('Rotorcraft', blank), _plT('ASEL', aselV), _plT('AMEL', amelV),
      ],
      rightGroups: [
        { l: '...and Class', s: 2 }, { l: 'Conditions of Flight', s: 4 }, { l: 'Approach', s: 1 }, { l: 'Holds', s: 1 },
        { l: 'Flight Simulator', s: 1 }, { l: 'As Flight Instructor', s: 1 }, { l: 'Type of Piloting Time', s: 4 }, { l: 'Total Duration', s: 1 },
      ],
      right: [
        _plX('ASES', blank), _plX('AMES', blank),
        _plX('Cross Country', blank), _plT('Night', ngt), _plX('Actual Instrument', blank), _plX('Simulated Instrument', blank),
        _plX('Type', apprType), _plX('Holds', blank),
        _plX('Flight Simulator', blank), _plX('As Flight Instructor', blank),
        _plX('Relief Officer', blank), _plX('Dual Received', blank), _plT('PIC', _plPicMin), _plT('SIC', _plSicMin),
        _plT('Total Duration', blk),
      ],
    },

    // ── Jeppesen Professional Pilot Log ──────────────────────────────────────────
    {
      id: 'jeppesen', name: 'Jeppesen', orientation: 'landscape',
      totalsLabels: ['Totals This Page', 'Amount Forwarded', 'Totals To Date'],
      certLines: ['I certify that the entries in this log are true,', '________________________________', "Pilot's Signature:"],
      leftGroups: [
        { l: 'Date', s: 1 }, { l: 'Aircraft Make and Model', s: 1 }, { l: 'Aircraft Ident', s: 1 }, { l: 'Route of Flight', s: 2 },
        { l: 'Total Duration of Flight', s: 1 }, { l: 'Aircraft Category and Class', s: 8 }, { l: 'Lndgs', s: 2 },
      ],
      left: [
        _plX('Date', dYMD),
        _plX('Make and Model', typ), _plX('Ident', reg),
        _plX('From', org), _plX('To', dst),
        _plT('Total Duration', blk),
        _plT('ASEL', aselV), _plX('ASES', blank), _plT('AMEL', amelV), _plX('AMES', blank), _plT('Jet', jetV), _plX('Rotorcraft', blank), _plX('Glider', blank), _plX('PCATD', blank),
        _plI('Day', dLd), _plI('Night', nLd),
      ],
      rightGroups: [
        { l: 'Conditions of Flight', s: 3 }, { l: 'App', s: 2 }, { l: 'Flight Simulator', s: 1 },
        { l: 'Type of Piloting Time', s: 6 }, { l: 'Remarks and Endorsements', s: 1 },
      ],
      right: [
        _plT('Night', ngt), _plX('Actual Instrument', blank), _plX('Simulated Instrument', blank),
        _plI('No', apprNum), _plX('Type', apprType),
        _plX('Flight Simulator', blank),
        _plX('Cross Country', blank), _plX('Solo', blank), _plT('PIC', _plPicMin), _plT('SIC', _plSicMin), _plX('Dual Received', blank), _plX('Flight Instructor', blank),
        _plX('Remarks', rmk),
      ],
    },

    // ── Compact 單頁（市面 condensed 版型：一航班一列、欄位精簡、不分左右）──────────────
    {
      id: 'compact', name: 'Compact', single: true,
      totalsLabels: ['TOTAL THIS PAGE', 'BROUGHT FORWARD', 'TOTAL TIME'],
      certLines: ['I certify that the entries in this log are true.', '________________________________', "PILOT'S SIGNATURE"],
      groups: [
        { l: 'DATE', s: 1 }, { l: 'FLIGHT', s: 1 }, { l: 'ROUTE', s: 2 }, { l: 'AIRCRAFT', s: 2 },
        { l: 'TIME', s: 4 }, { l: 'LANDINGS', s: 2 }, { l: 'REMARKS', s: 1 },
      ],
      cols: [
        _plX('Date', dSlash), _plX('Flight', fno), _plX('From', org), _plX('To', dst),
        _plX('Type', typ), _plX('Reg', reg),
        _plT('Total', blk), _plT('Night', ngt), _plT('PIC', _plPicMin), _plT('SIC', _plSicMin),
        _plI('Day', dLd), _plI('Night', nLd),
        _plX('Remarks', rmkAppr),
      ],
    },
...
function _plLbCollectChars(fmt, src, extra) {
  ...
  add(extra || '');
  var cols = fmt.single ? (fmt.cols || []) : (fmt.left || []).concat(fmt.right || []);
  (fmt.single ? (fmt.groups || []) : (fmt.leftGroups || []).concat(fmt.rightGroups || [])).forEach(function (g) { add(g.l); });
  cols.forEach(function (c) { add(c.h); });
  for (var i = 0; i < src.length; i++) { for (var j = 0; j < cols.length; j++) { add(cols[j].get(src[i])); } }
...
function _plGenLogbookPdf(formatId, opts) {
  opts = opts || {};
  // V2.4.20：舊格式 id（generic/faa/icao）→ 對映到新版面，避免舊狀態/書籤靜默掉成別的格式（codex P2）。
  var ALIAS = { generic: 'general', faa: 'jeppesen', icao: 'general' };
  formatId = ALIAS[formatId] || formatId;
  var r = _plReportRange();
  ...
  var single = !!fmt.single;   // Compact 單頁：不分左右
  var L = single ? fmt.cols : fmt.left, R = single ? null : fmt.right;
  var allCols = single ? fmt.cols : fmt.left.concat(fmt.right);
  var pilot = _plSelfName();
  var ASCII = ' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~';
  var staticTxt = ASCII + ' PROFESSIONAL PILOT LOGBOOK Entries from Through PAGE REPORT to Generated by reference times in HH:MM UTC ' +
    fmt.name + ' ' + pilot + ' ' + r.from + ' ' + r.to + ' ' + (fmt.totalsLabels || []).join(' ') + ' ' + (fmt.certLines || []).join(' ');
  var allChars = _plLbCollectChars(fmt, src, staticTxt);
  var needCJK = _plLbHasCJK(allChars);   // 由資料/姓名決定（ASCII 使用者不強制下載中文字型）
  var COVER_CJK = '飛行紀錄簿';            // 封面副標；只有在已需 CJK 時順便收進 subset，否則略過該行
  _plToast('產生 PDF… Generating');
  _plLoadJsPdf().then(function () {
    var fontP = needCJK
      ? _plApi('/api/pilot-log/font-subset', { method: 'POST', body: { chars: allChars + COVER_CJK } })
    ...
    fontP.then(function (fontB64) {
      ...
      function cjkCell(data) { if (CJK && _plLbHasCJK((data.cell.text || []).join(''))) { data.cell.styles.font = CJK; data.cell.styles.fontStyle = 'normal'; } }
      var ob = (_pl.stats && _pl.stats.opening) || [];   // 起始累計（封面 Total Time 與下面結轉 carry 共用）

      // ── 封面（中性持有人前頁，不抄 LogTen）：外框 + 標題置上 / 姓名置中 / 資訊區置下，整頁有結構不空蕩 ──
      if (opts.cover !== false) {
        var cx = pageW / 2;
        // 本區間時數 + 累計到日（起始累計 + 全部 app 內航班）
        var qfly = function (e) { return !(e.is_deadhead || e.status === 'roster_removed' || !e.in_utc); };
        var periodMin = 0; for (var pi = 0; pi < src.length; pi++) periodMin += src[pi].block_minutes || 0;
        var cumMin = 0; ob.forEach(function (o) { cumMin += o.total_minutes || 0; });
        (_pl.aircraftEntries || []).forEach(function (e) { if (qfly(e)) cumMin += e.block_minutes || 0; });
        // 雙線外框（讓封面像一頁封面，不是文字漂浮）
        doc.setDrawColor(45, 65, 95); doc.setLineWidth(0.9); doc.rect(13, 13, pageW - 26, pageH - 26);
        doc.setDrawColor(120, 140, 165); doc.setLineWidth(0.3); doc.rect(16.5, 16.5, pageW - 33, pageH - 33);
        // 標題（上 1/3）
        doc.setFontSize(30); _plLbFont(doc, 'PILOT LOGBOOK', CJK, 'bold'); doc.setTextColor(30, 45, 70);
        // jsPDF 置中沒把 charSpace 算進寬度→會偏；自己算含字距寬度再靠左定位置中
        doc.setCharSpace(3);
        var _ttl = 'PILOT LOGBOOK', _ttlW = doc.getTextWidth(_ttl) + 3 * (_ttl.length - 1);
        doc.text(_ttl, cx - _ttlW / 2, pageH * 0.30); doc.setCharSpace(0);
        if (CJK) { doc.setFontSize(15); _plLbFont(doc, COVER_CJK, CJK, 'normal'); doc.setTextColor(95); doc.text(COVER_CJK, cx, pageH * 0.30 + 11, { align: 'center' }); }
        // 持有人姓名（中段，大字）
        doc.setFontSize(25); _plLbFont(doc, pilot, CJK, 'bold'); doc.setTextColor(20);
        doc.text(pilot || '', cx, pageH * 0.47, { align: 'center' });
        doc.setFontSize(8.5); doc.setFont('helvetica', 'normal'); doc.setTextColor(130); doc.setCharSpace(1.2);
        var _lh = 'LOGBOOK HOLDER', _lhW = doc.getTextWidth(_lh) + 1.2 * (_lh.length - 1);
        doc.text(_lh, cx - _lhW / 2, pageH * 0.47 + 7.5); doc.setCharSpace(0);
        // 資訊區（下 1/3）：標籤/值靠左對齊，整塊置中
        var boxW = 168, boxX = cx - boxW / 2, valX = boxX + 46, ry = pageH * 0.66, rowGap = 12;
        function infoRow(label, value) {
          doc.setFontSize(11); doc.setTextColor(60); doc.setFont('helvetica', 'bold'); doc.text(label, boxX, ry);
          doc.setTextColor(25); doc.setFont('helvetica', 'normal');
          if (value) { doc.text(value, valX, ry); }
          else { doc.setDrawColor(160); doc.setLineWidth(0.3); doc.line(valX, ry + 1, valX + 62, ry + 1); }   // 底線長度跟下方資料差不多，不拉到整框
          ry += rowGap;
        }
        if (opts.license !== false) infoRow('License No.', '');
        infoRow('Period', r.from + '   —   ' + r.to);
        if (opts.totalTime !== false) infoRow('Total Time', 'this period ' + _plMinToHHMM(periodMin) + '    ·    to date ' + _plMinToHHMM(cumMin));
        doc.setTextColor(0);
      }

      // ── 起始累計 + 區間前航班 → 第 1 頁承前頁 carry（over allCols）──
      // 起始累計每筆「依機型」存（含 aircraft_type）→ 合成保留機型的 pseudo-entry，讓 JET/AMEL/ASEL/Multi-Pilot 等
      //   依分類正確結轉（codex P1：之前合成單一無機型 op，分類欄全變 0、低報歷史）。
      // 起始累計（previous experience）沒有機組人數資訊（API 只回時數+機型）→ 多機組與否「依機型推定」：
      //   operating_crew:0 → isMP() 退化成「是不是噴射機」。噴射機結轉 → Multi-Pilot（航線屬實）；非噴射 → 單駕 SE/ME。
      //   這是無機組資料下最有根據的分法（少數「多機組渦輪螺旋槳」結轉會被當單駕，屬無法判定的邊界）。
      var openPseudos = ob.map(function (o) {
        return { block_minutes: o.total_minutes || 0, pic_minutes: o.pic_minutes || 0, sic_minutes: o.sic_minutes || 0, night_minutes: o.night_minutes || 0,
                 aircraft_type: o.aircraft_type || '', operating_crew: 0, day_landings: 0, night_landings: 0, day_takeoffs: 0, night_takeoffs: 0 };
      });
      var preRange = (_pl.aircraftEntries || []).filter(function (e) {
        if (e.is_deadhead || e.status === 'roster_removed' || !e.in_utc) return false;
        var fd = String(e.flight_date || '').slice(0, 10);
        return fd && r.from && fd < r.from;
      });
      var carry = allCols.map(function (c) {
        if (!c.val) return 0;
        var s = 0, k;
        for (k = 0; k < openPseudos.length; k++) s += c.val(openPseudos[k]) || 0;
        for (k = 0; k < preRange.length; k++) s += c.val(preRange[k]) || 0;
        return s;
      });
      // 起始累計 API 不回起降次數（只回時數）→ 有起始累計時，起降的「累計」不完整 → 結轉列（承前/累計）的計數欄留空，只在本頁顯示。
      var cumCounts = (ob.length === 0);
      var TL = fmt.totalsLabels || ['TOTAL THIS PAGE', 'TOTAL FROM PREVIOUS PAGES', 'TOTAL TIME'];
      var CERT = fmt.certLines || ['I certify that the entries in this log are true.', '________________________________', "PILOT'S SIGNATURE"];
      function firstValIdx(cols) { var i = 0; while (i < cols.length && !cols[i].val) i++; return i < cols.length ? i : 1; }
      function remIdx(cols) { for (var i = 0; i < cols.length; i++) { if (/remark/i.test(cols[i].h)) return i; } return -1; }

      // 三列結轉（本頁/承前/累計）；withLabel=左頁(有標籤)、右頁無標籤。
      //   認證簽名＝REMARKS 欄「一個大格」：只在第一列放 {rowSpan:3} 的合併儲存格（認證句＋簽名線＋PILOT'S SIGNATURE 同一框），
      //   後兩列略過該欄（被 rowSpan 蓋住）—— 不再是三小格（照紙本一大格）。
      function totalsRows(cols, offset, pageSum, brought, toDate, withLabel) {
        var ft = firstValIdx(cols), rc = remIdx(cols);
        function mk(label, vals, isCum, ri) {
          var row = [], startCi = 0;
          if (withLabel) {
            var span = (rc >= 0 && rc < ft) ? rc : ft; if (span < 1) span = 1;
            row.push({ content: label, colSpan: span, styles: { halign: 'right', fontStyle: 'bold' } });
            startCi = span;
          }
          for (var ci = startCi; ci < cols.length; ci++) {
            var c = cols[ci], gi = offset + ci;
            if (ci === rc) {
              // 認證句在上 → 留白（給簽名下筆空間）→ 簽名線靠下 → 線下放標籤。CERT=[句, 線, 標籤]。
              if (ri === 0) row.push({ content: CERT[0] + '\n\n\n' + CERT[1] + '\n' + CERT[2], rowSpan: 3, styles: { halign: 'left', valign: 'top', fontStyle: 'normal', fillColor: [255, 255, 255] } });
              continue;   // ri 1,2：被 rowSpan 蓋住，不放儲存格
            }
            if (!c.val) { row.push(''); continue; }
            if (isCum && c.tot !== 'time' && !cumCounts) { row.push(''); continue; }
            row.push(c.tot === 'time' ? _plMinToHHMM(vals[gi]) : String(vals[gi]));
          }
          return row;
        }
        return [mk(TL[0], pageSum, false, 0), mk(TL[1], brought, true, 1), mk(TL[2], toDate, true, 2)];
      }

      // 每頁行數 & 列高 —— 字級加大到 8pt（完整版）/ 9pt（Compact），列高貼合字體（不再格子遠大於字）。
      //   rowH=每列高(mm)；ROWS 由實際頁高反推（扣 startY + 底部 + 表頭2列 + 結轉3列），列高用 minCellHeight=rowH 撐滿。
      var rowH = 6;
      // 表頭（群組列+欄名列，空欄可能換 2 行）+ 三列結轉（含較高的簽名認證格）實際會比 rowH 高 →
      //   保守預留 headBlock/totBlock，避免滿頁時最後幾列或頁尾超出可印範圍（codex P2）。寧可多分一頁。
      var headBlock = 18, totBlock = 24;
      var ROWS = Math.max(8, Math.floor((pageH - 12 - 10 - headBlock - totBlock) / rowH));
      var availW = pageW - 16;   // 左右各 8mm margin
      // jsPDF 建構時已有第 1 頁：封面有開→畫在第 1 頁，第一個內頁要 addPage；封面關掉→第 1 頁還空著，
      //   第一個內頁直接用它（不可再 addPage，否則開頭多一張空白頁）。
      var usedFirstPage = (opts.cover !== false);

      // 渲染一個實體頁（左或右）
      function renderPhys(cols, groups, offset, chunk, pageSum, brought, toDate, withLabel, pageLabel) {
        var pageMany = cols.length > 16;
        var fs = pageMany ? 8 : 9, hfs = pageMany ? 6.5 : 7.5;
        var rc = remIdx(cols);
        // 依使用者實際資料逐欄量寬：空欄縮窄、有資料欄分到剩餘寬度（用整份 src，跨頁欄寬一致）
        var widths = _plLbColWidths(cols, src, availW);
        // 全表置中（一致）；只有 Remarks 自由文字維持靠左
        var columnStyles = {}; widths.forEach(function (w, i) { columnStyles[i] = { cellWidth: w, halign: /remark/i.test(cols[i].h) ? 'left' : 'center' }; });
        var head = [groups.map(function (g) { return { content: g.l, colSpan: g.s }; }), cols.map(function (c) { return c.h; })];
        var body = chunk.map(function (e) { return cols.map(function (c) { return c.get(e); }); });
        body = body.concat(totalsRows(cols, offset, pageSum, brought, toDate, withLabel));
        if (usedFirstPage) doc.addPage(); else usedFirstPage = true;   // 封面關掉時第一個內頁沿用第 1 頁，避免開頭空白頁
        doc.setFontSize(8); _plLbFont(doc, pilot, CJK, 'bold'); doc.setTextColor(20);
        doc.text((pilot ? pilot + '    ' : '') + fmt.name, 8, 9); doc.setTextColor(0);
        doc.autoTable({
          head: head, body: body, startY: 12, theme: 'grid', columnStyles: columnStyles, tableWidth: availW,
          styles: { font: 'helvetica', fontSize: fs, cellPadding: 1, minCellHeight: rowH, overflow: 'ellipsize', valign: 'middle', lineWidth: 0.1, lineColor: [150, 160, 175] },
          headStyles: { fillColor: [40, 60, 90], textColor: 255, fontSize: hfs, minCellHeight: rowH, halign: 'center', fontStyle: 'bold', overflow: 'linebreak', valign: 'middle' },
          margin: { left: 8, right: 8 },
          didParseCell: (function (cl, rcc) {
            return function (data) {
              if (data.section === 'body' && data.row.index >= cl) {   // 三列結轉塊
                data.cell.styles.fontStyle = 'bold';
                data.cell.styles.fillColor = (data.row.index === cl + 2) ? [210, 224, 240] : [233, 239, 247];
                if (rcc >= 0 && data.column.index === rcc) {   // 認證簽名合併格：左對齊、置頂、可換行、白底、不粗體
                  data.cell.styles.fontStyle = 'normal'; data.cell.styles.fillColor = [255, 255, 255];
                  data.cell.styles.halign = 'left'; data.cell.styles.valign = 'top'; data.cell.styles.overflow = 'linebreak'; data.cell.styles.fontSize = hfs;
                }
              }
              cjkCell(data);
            };
        ...
        doc.text('PAGE ' + pageLabel + '   ( REPORT: ' + r.from + ' - ' + r.to + ' )', 8, pageH - 5);
        doc.setTextColor(0);
      }

      // 逐頁：完整版＝一個 chunk 跨左右兩頁；Compact 單頁＝一個 chunk 一頁。
      var nPages = Math.max(1, Math.ceil(src.length / ROWS));
      for (var pg = 0; pg < nPages; pg++) {
        var chunk = src.slice(pg * ROWS, pg * ROWS + ROWS);
        var pageSum = allCols.map(function (c) { var s = 0; if (c.val) { for (var j = 0; j < chunk.length; j++) s += (c.val(chunk[j]) || 0); } return s; });
        var brought = carry.slice();
        var toDate = carry.map(function (v, i) { return v + pageSum[i]; });
        carry = toDate;
        if (single) {
          renderPhys(L, fmt.groups, 0, chunk, pageSum, brought, toDate, true, String(pg + 1));
        } else {
          renderPhys(L, fmt.leftGroups, 0, chunk, pageSum, brought, toDate, true, (pg + 1) + 'A');
          renderPhys(R, fmt.rightGroups, L.length, chunk, pageSum, brought, toDate, false, (pg + 1) + 'B');
        }
      }
...
function _plOpenLogbookDialog() {
  ...
  var _ord = ['general', 'easa', 'asa', 'jeppesen', 'compact'];   // General 排第一（預設）
  var _ofmts = _ord.map(function (id) { return fmts.filter(function (f) { return f.id === id; })[0]; }).filter(Boolean);
  fmts.forEach(function (f) { if (_ord.indexOf(f.id) < 0) _ofmts.push(f); });
  var fopt = _ofmts.map(function (f) { return '<option value="' + f.id + '">' + _plEsc(f.name) + '</option>'; }).join('');
  ...
  ov.innerHTML =
    '<div style="background:var(--card);border-radius:14px;max-width:420px;width:100%;padding:18px;box-shadow:0 10px 40px rgba(0,0,0,.45);max-height:90vh;overflow:auto">' +
      ...
      '<label style="display:flex;align-items:center;gap:7px;margin-top:12px;font-size:.82em;cursor:pointer"><input type="checkbox" id="pl-lb-cover" checked> 產生封面頁 Cover page</label>' +
      '<label style="display:flex;align-items:center;gap:7px;margin-top:6px;font-size:.8em;cursor:pointer;padding-left:20px;color:var(--muted)"><input type="checkbox" id="pl-lb-license" checked> 封面顯示 License No. 欄</label>' +
      '<label style="display:flex;align-items:center;gap:7px;margin-top:6px;font-size:.8em;cursor:pointer;padding-left:20px;color:var(--muted)"><input type="checkbox" id="pl-lb-total" checked> 封面顯示 Total Time（本區間 + 累計）</label>' +
      '<div style="font-size:.62em;color:var(--muted);margin-top:8px">完整版（General/EASA/ASA/Jeppesen）＝照紙本左右跨頁橫式；Compact＝單頁精簡。</div>' +
...
function _plLogbookGo() {
  var fmt = (document.getElementById('pl-lb-fmt') || {}).value || 'general';
  var paper = (document.getElementById('pl-lb-paper') || {}).value || 'a4';
  var cover = (document.getElementById('pl-lb-cover') || { checked: true }).checked;
  var license = (document.getElementById('pl-lb-license') || { checked: true }).checked;
  var totalTime = (document.getElementById('pl-lb-total') || { checked: true }).checked;
  _plCloseLogbookDialog();
  _plGenLogbookPdf(fmt, { paper: paper, cover: cover, license: license, totalTime: totalTime });
}
