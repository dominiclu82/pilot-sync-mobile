...
<div class="tab-bar">
  ...
  <div class="tab-btn tab-util" style="flex-direction:row;gap:0">
    ...
    <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">
      ...
      <span style="font-size:.55em;color:var(--muted);line-height:1;opacity:.7;cursor:pointer" onclick="showAbout()">V6.179</span>
    </div>
  </div>
</div>
...
<div id="about-overlay" class="install-overlay" style="display:none" onclick="if(event.target===this)closeAbout()">
  <div class="install-card">
    ...
    <div style="font-size:.78em;font-weight:700;margin-bottom:6px" id="about-version">V6.179</div>
    <div style="font-size:.72em;color:var(--muted);margin-bottom:10px;line-height:1.5;text-align:left">
      <div>修正排隊倒數重複跳回；修正手機 Google Calendar 事件面板被切掉</div>
      <div>Fix queue countdown resetting; fix mobile Google Calendar events panel cut off</div>
    </div>
    <div style="font-size:.78em;font-weight:700;color:var(--muted);margin-bottom:6px">V6.177</div>
    <div style="font-size:.72em;color:var(--muted);margin-bottom:10px;line-height:1.5;text-align:left">
      <div>同步排隊倒數即時遞減；開始同步後清除排隊訊息；預估時間改為 60 秒/人；所有分頁取消 iOS 視窗回彈</div>
      <div>Sync queue countdown updates every second; clear queue msg on running; ETA 60s/person; disable iOS overscroll bounce on all panels</div>
    </div>
