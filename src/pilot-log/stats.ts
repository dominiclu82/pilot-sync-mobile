...
export async function getTotals(userId: string): Promise<Totals> {
  ...
  const r = await pool.query(
    `SELECT
     ...
     WHERE user_id = $1 AND status = 'confirmed' AND is_deadhead IS NOT TRUE`,
    [userId]
  );
  const row = r.rows[0] || {};
  return {
...
export async function getByAircraftType(userId: string): Promise<ByTypeRow[]> {
  ...
  const r = await pool.query(
    `SELECT aircraft_type,
            ...
            COUNT(*)::int                         AS entry_count
     FROM pilot_log_entries
     WHERE user_id = $1 AND status = 'confirmed' AND aircraft_type IS NOT NULL AND is_deadhead IS NOT TRUE
