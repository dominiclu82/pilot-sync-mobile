...
export async function getTotals(userId: string): Promise<Totals> {
  ...
  const r = await pool.query(
    `SELECT
       ...
       COALESCE(SUM(CASE WHEN position = 'PIC' THEN block_minutes ELSE 0 END), 0)::int    AS pic_minutes,
       COALESCE(SUM(CASE WHEN position = 'SIC' THEN block_minutes ELSE 0 END), 0)::int    AS sic_minutes,
...
export async function getByAircraftType(userId: string): Promise<ByTypeRow[]> {
  ...
  const r = await pool.query(
    `SELECT aircraft_type,
            ...
            COUNT(*)::int                         AS entry_count
     FROM pilot_log_entries
     WHERE user_id = $1 AND status = 'confirmed' AND aircraft_type IS NOT NULL
