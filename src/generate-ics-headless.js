...
export async function generateICSHeadless(targetYear, targetMonth, jxCredentials, icsPath, onLog) {
    ...
    try {
        ...
        if (diff !== 0) {
            ...
            for (let i = 0; i < Math.abs(diff); i++) {
                ...
                if (!clicked) {
                    try {
                        const handle = await page.evaluateHandle((dir) => {
        ...
        while (true) {
            ...
            for (let j = 0; j < headerDivs.length; j++) {
                ...
                if (txt === 'From')
                    reportTimeStr = (await headerDivs[j + 1]?.textContent())?.trim() || '';
                else if (txt === 'To')
                    endTimeStr = (await headerDivs[j + 1]?.textContent())?.trim() || '';
            }
            ...
            if (isFlightDuty) {
                ...
                for (const item of items) {
                    const d = await item.evaluate((el) => ({
                        ...
                        origin: el.querySelector('.startLocnId')?.textContent?.trim() ?? '',
                        dest: el.querySelector('.endLocnId')?.textContent?.trim() ?? '',
                        depTimeLocal: el.querySelector('.startTimeLocal')?.textContent?.trim() ?? '',
                        depTimeUtc: el.querySelector('.startTimeUTC, .startTimeUtc')?.textContent?.trim() ?? '',
                        arrTimeLocal: el.querySelector('.endTimeLocal')?.textContent?.trim() ?? '',
                        arrTimeUtc: el.querySelector('.endTimeUTC, .endTimeUtc')?.textContent?.trim() ?? '',
                        position: el.querySelector('.position')?.textContent?.trim() ?? '',
                        flightTime: el.querySelector('.flightTime')?.textContent?.trim().replace(/^FT\s*/, '') ?? '',
                        workCode: el.querySelector('.workDuty')?.firstChild?.textContent?.trim() ?? '',
                    }));
                    if (d.flightNo) {
                    ...
                    if (d.dest && d.dest !== 'TPE')
                        outstation = d.dest;
                }
                if (!outstation && flightNos.length)
                    outstation = 'TPE';
                ...
                for (const card of dutyCards) {
                    ...
                    if (d && d.workCode) {
                        var datePrefix = d.date || '';
                        var fullStart = datePrefix && d.startTime ? datePrefix + ' ' + d.startTime : d.startTime;
                        var fullEnd = datePrefix && d.endTime ? datePrefix + ' ' + d.endTime : d.endTime;
                        flights.push({
            ...
            return result;
                    ...
                    if (flightHeaders.length === 0) {
                        const data = await page.evaluate(parseCrewText);
                    ...
                    for (const f of flights) {
                        ...
                        if (!f.crew || f.crew.length === 0) {
                            for (const [label, crew] of Object.entries(crewData)) {
                                if (!f.date) continue;
                                // f.date = "2026.Feb.09", label = "Training - Feb.09"
                                const datePart = f.date.split('.').slice(1).join('.');  // "Feb.09"
